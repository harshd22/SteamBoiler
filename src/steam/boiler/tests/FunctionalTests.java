...
public class FunctionalTests {
  ...
  private void test_normal_operation(int time, int numberOfPumps) {
    ...
    if (model.getBoiler().getWaterLevel() > config.getMaximalNormalLevel()) {
      fail("Water level above normal maximum (after " + time + "s with " + numberOfPumps
          + " pumps)");
    }
    if (model.getBoiler().getWaterLevel() < config.getMinimalNormalLevel()) {
      fail("Water level below normal minimum (after " + time + "s with " + numberOfPumps
          + " pumps)");
    }
  }
  ...
  public void test_degraded_operation_03() {
    ...
    // full exhaust.
    config.setNumberOfPumps(2, config.getPumpCapacity(0));
    SteamBoilerController controller = new SteamBoilerController(config);
    PhysicalUnits model = new PhysicalUnits.Template(config).construct();
    ...
    // System should immediately notice and respond to this.
    clockOnceExpecting(controller, model, atleast(MODE_degraded, PUMP_FAILURE_DETECTION(0)));
  }
  ...
  public void test_degraded_operation_04() {
    ...
    // System should immediately notice and respond to this.
    clockOnceExpecting(controller, model,
        atleast(MODE_degraded, PUMP_CONTROL_FAILURE_DETECTION(0)));
  }
  ...
  public void test_degraded_operation_05() {
    ...
    config.setNumberOfPumps(3, config.getPumpCapacity(0));
    SteamBoilerController controller = new SteamBoilerController(config);
    PhysicalUnits model = new PhysicalUnits.Template(config).construct();
    model.setMode(PhysicalUnits.Mode.WAITING);
  ...
  private void test_degraded_operation(SteamBoilerController controller,
    ...
    if (model.getBoiler().getWaterLevel() > config.getMaximalNormalLevel()) {
      fail("Water level above normal maximum (after " + time + "s with " + config.getNumberOfPumps()
          + " pumps)");
    }
    if (model.getBoiler().getWaterLevel() < config.getMinimalNormalLevel()) {
      fail("Water level below normal minimum (after " + time + "s with " + config.getNumberOfPumps()
          + " pumps)");
    }
  }
